...
public class NeuralNetwork {
    ...
    public NeuralNetwork(NeuralNetwork nn) {
        ...
        this.weights = new SimpleMatrix[hiddenNodes.length + 1];
        this.biases = new SimpleMatrix[hiddenNodes.length + 1];
    ...
    private void initializeWeights() {
        weights = new SimpleMatrix[hiddenNodes.length + 1];
        ...
        for (int i = 0; i < weights.length; i++) {
            if (i == 0) { // 1st weights that connects inputs to first hidden layer
                weights[i] = SimpleMatrix.random64(hiddenNodes[i], inputNodes, -1, 1, random);
            } else if (i == weights.length - 1) { // last weights that connect last hidden layer to output
                weights[i] = SimpleMatrix.random64(outputNodes, hiddenNodes[i - 1], -1, 1, random);
            } else { // everything else
                weights[i] = SimpleMatrix.random64(hiddenNodes[i], hiddenNodes[i - 1], -1, 1, random);
            }
        }
    }

    private void initializeBiases() {
        biases = new SimpleMatrix[hiddenNodes.length + 1];
        ...
        for (int i = 0; i < biases.length; i++) {
            if (i == biases.length - 1) { // bias for last layer (output layer)
                ...
                biases[i] = SimpleMatrix.random64(hiddenNodes[i], 1, -1, 1, random);
            }
        }
    }
    ...
    public void train(double[] inputArray, double[] targetArray) {
        if (inputArray.length != inputNodes) {
            ...
            SimpleMatrix layers[] = new SimpleMatrix[hiddenNodes.length + 2];
            layers[0] = input;
            for (int j = 1; j < hiddenNodes.length + 2; j++) {
                layers[j] = calculateLayer(weights[j - 1], biases[j - 1], input, activationFunction);
                input = layers[j];
            }

            for (int n = hiddenNodes.length + 1; n > 0; n--) {
                // Calculate error
    ...
    public int[] getDimensions(){
        return new int[]{inputNodes, hiddenNodes.length, outputNodes};
    }
